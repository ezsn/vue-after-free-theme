...
// Top red border strip (only visible on selected row)
var topBorder = new Image({
  url: selectedButtonImg,
  x: btnX,
  y: btnY - 1,
  width: buttonWidth,
  height: 6,
...
topBorder.alpha = 1.0;
topBorder.borderColor = 'rgb(255, 60, 60)';
topBorder.borderWidth = 6; // fills the strip (small side borders are negligible)
topBorders.push(topBorder);
jsmaf.root.children.push(topBorder);
  ...
  for (var i = 0; i < buttons.length; i++) {
    ...
    // Row box: selected = white fill, unselected = transparent
    b.url = selectedButtonImg;
    b.alpha = isSel ? 1.0 : 0.0;
    b.borderColor = 'transparent';
    b.borderWidth = 0;

    // Top red line only on selected
    if (topBorders && topBorders[i]) {
      topBorders[i].visible = isSel;
      topBorders[i].x = b.x;
      topBorders[i].y = b.y - 1;
      topBorders[i].width = b.width;
    }
