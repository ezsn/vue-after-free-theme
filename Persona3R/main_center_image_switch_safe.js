(function () {
  ...
  var textImageBase = 'file://../download0/themes/hacker/img/';
  var currentButton = 0;
  var buttons = [];
  var buttonTexts = [];
  var buttonMarkers = [];
...
// UI-only: center image that swaps based on current selection (Jailbreak/Payload/Config/Exit)
var centerImageW = 1000;
var centerImageH = 560;
function getCenterImageUrl(opt) {
  if (!opt || !opt.imgKey) return textImageBase + 'Jailbreak.png';
  // Map existing keys to your filenames
...
var centerImage = new Image({
  url: getCenterImageUrl(menuOptions[currentButton]),
...
jsmaf.root.children.push(centerImage);
  ...
  function updateHighlight() {
    ...
    // UI-only: update center image
    if (centerImage) {
      centerImage.url = getCenterImageUrl(menuOptions[currentButton]);
    }
